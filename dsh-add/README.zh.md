...
<img src="assets/logo/logo-readme-v6-400.png" alt="dsh-add：Q 版 DeepSeek 鲸鱼娘捧着一个插件方块" width="300">
...
**项目 logo 是 `logo-readme-v6*.png`**（低饱和水彩、白底已抠成透明、四周渐隐，README 顶部用的就是它）：

| 文件 | 说明 |
|---|---|
| `logo-readme-v6.png` | 主图 1024×1536 |
| `logo-readme-v6-800/400/200.png` | 等比缩放，400 是 README 实际使用的宽度 |
| `logo-readme-v6-square-512/128.png` | 方形版（透明底居中），用于头像/包页 |

四周渐隐是为了同时适配 GitHub 的浅色与深色主题：白底抠成透明后，再在四周 9% 的范围做 alpha 渐隐，让画面化进页面背景。

**清除发丝间封闭纸底时踩过的两个坑**（这版 logo 返工最多的一处）：

1. **洪水填充必须在区域限制之前做**。若先把范围限到"头部两侧"，发丝会把外部背景与画布边缘隔断，整块外部背景被误判为封闭（实测报出 90k 与 116k 像素的假候选）。
2. **判据不能用位置和面积猜**。第一版如此，结果把腰间的**白色蕾丝**一起抠掉了（蕾丝白与纸底残留白同为中性白：min 通道 253–254、`|R−B| ≤ 1`，颜色上完全不可分）。可靠判据是**块周围是否被深色发丝包围**：给每个候选块取一圈环带，环带内「暗且偏蓝」的像素占比 ≥55% 才清除。实测真正的发丝间隙是 60–91%，被误删的蕾丝块只有 4–43%，区分度充分。

最终清除 5 块共 3657 px（比上一版保守：上一版清了 9557 px 并误伤蕾丝）。另需知道的一处固有损失：水彩的细轮廓线通道接近纸色，去背时会一并被抠掉，全图约 14857 px，集中在呆毛顶端（3285 px）与鞋部（约 9150 px）。
